...
| 3  | Hash functions & MACs     | MD5 through SHA3-256; the SHA-256 compression function round by round; the length extension attack; HMAC; avalanche; the k-anonymity query pattern; Merkle trees with inclusion proofs; Lamport one-time signatures and the sign-twice forgery | FIPS 180-4, FIPS 202, RFC 2104, RFC 6962 |
...
| 5  | Randomness                | RANDU's lattice in 3D, NIST monobit/runs tests, von Neumann extraction of pointer and CPU-jitter entropy, a ChaCha20 CSPRNG from a seed, modulo bias vs rejection sampling, Fisher–Yates, the birthday bound, Monte Carlo π, a commit–reveal coin flip, the PRG distinguishing game, a provably fair casino roll | NIST SP 800-22 |
| 6  | Public-key cryptography   | RSA (PKCS#1 v1.5, OAEP, PSS, key components); finite-field DH with RFC 7919 groups; X25519 agreement; Ed25519 signatures; a toy elliptic curve plotted point by point with ECDH and a live discrete-log break | RFC 8017, RFC 7919, RFC 7748, RFC 8032 |
...
| 12 | Secret sharing            | Shamir's threshold scheme: split, reconstruct by Lagrange interpolation, and the proof that k−1 shares fit every possible secret | Shamir 1979 |
| 13 | Encodings                 | Base64, Base64url, hex, percent, binary; code-point and UTF-8 byte view                       | RFC 4648                         |
| 14 | Benchmark                 | Identical chained SHA-256 workload in WebAssembly, JavaScript and WebCrypto; expected break times for DES through AES-256 and RSA/ECC at SP 800-57 equivalent strengths, from your measured rate up to the whole Bitcoin network | NIST SP 800-57 |
