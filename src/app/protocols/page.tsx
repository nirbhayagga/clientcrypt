  ...
  return (
    <Panel title="Wi-Fi: WPA2-PSK key derivation" refs={['IEEE 802.11i', 'RFC 8018']}>
      ...
      <Note title="Why home Wi-Fi passwords get cracked">
        ...
        can test passphrase guesses offline at 4096 SHA-1 iterations each — cheap for a GPU. The defence is exactly §7:
...
function WireGuardPanel({ ready }: { ready: boolean }) {
  ...
  return (
    <Panel title="WireGuard handshake" refs={['Noise IKpsk2', 'Curve25519', 'ChaCha20-Poly1305', 'BLAKE2s']}
      ...
      <Note title="Compared with TLS (§8)">
        ...
        The chaining-key / hash ladder shown in §8 for HKDF-SHA256 runs here with HMAC-BLAKE2s. Encrypted fields and derived keys differ on
...
export default function ProtocolsPage() {
  ...
  return (
    <Page kicker="§9 · Applied protocols" title="Protocols in the field"
      lede="Four places the primitives from §1–§8 turn up between real machines: your Wi-Fi password, the six digits from an authenticator app, the token in an Authorization header, and a VPN tunnel.">
      <Status state={state} />
